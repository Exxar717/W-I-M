...
const getaProduct = asyncHandler(async (req, res) => {
  const { id } = req.params;
  try {
    const findProduct = await Product.findById(id).populate('color').populate({path: "ratings.postedby", select: "firstname"});
...
const getAllProduct = asyncHandler(async (req, res) => {
  try {
    ...
    if (req.query.page) {
      ...
      if (skip >= productCount) throw new Error("NO EXIST CABRON");
    }
